...
class TUIErrorHandler:
    """Enhanced error handling for TUI components"""
    
    @staticmethod
    def safe_execute(func, *args, default=None, error_prefix="Operation failed", **kwargs):
        """Safely execute TUI operations with error handling"""
        try:
    ...
    @staticmethod
    def validate_input(data: str, input_type: str = "safe_string") -> tuple:
        """Validate TUI input using main sanitizer"""
        try:
            # Use the main script's sanitizer if available
            import bl4ckc3ll_p4nth30n as main
            return main.sanitize_user_input(data, input_type)
        except ImportError:
            # Fallback validation
...
class TargetManagement(Static):
    ...
    def validate_target(self, target: str) -> bool:
        """Enhanced target validation using TUIErrorHandler"""
        # First, sanitize the input
        is_valid, sanitized_target = TUIErrorHandler.validate_input(target, "safe_string")
        if not is_valid:
            return False
        ...
        try:
            ...
            ipaddress.ip_address(sanitized_target)
            return True
        except ValueError:
            pass
        ...
        return bool(re.match(domain_pattern, sanitized_target))
...
class ReportViewer(Static):
    ...
    def compose(self) -> ComposeResult:
        ...
        # Reports will be loaded after mounting

    def on_mount(self) -> None:
        """Called when the widget is mounted"""
        self.call_later(self.load_reports)
